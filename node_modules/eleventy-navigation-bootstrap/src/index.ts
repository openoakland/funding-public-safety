...
const toBootstrapNav = (pages: Array<navigationPage>, options: navigationOptions = {}): string  => {

  function navItem(entry: navigationPage): string {

    function activeClass(): string {
      const hasActiveChildren = options.activeKey === entry.key // Is active itself or has active children …
            || !!entry.children.find((child: navigationPage): boolean => child.key === options.activeKey)
    ...
    const linkActive = options.activeKey === entry.key || entry.doorpageLabel ? 
          ...
          : `<a href="${urlFilter(entry.url)}" class="${isChildList ? options.listChildItemLinkClass : options.listItemLinkClass}" role="menuitem">${entry.title}</a>` 

        , linkDropdown = `${toBootstrapNav(entry.children, Object.assign(options, { isChildList: entry.key, parentEntry: entry }))}`

        , linkHasChildren = entry.children && entry.children.length ?
  ...
  function doorpageItem(entry: navigationPage): string {
